...
import { streamSSEResponse, createSSEHeaders } from './streaming.js';
...
async function handleOpenAIRequest(
  ...
  if (!decision.shouldIntercept) {
    stats.passedThrough++;
    return forwardOpenAIPassthrough(config.cloud.openai_base, apiKey, body, c);
  }
  ...
  try {
    ...
    const cloudResponse = await openai.forwardToCloud(config.cloud.openai_base, apiKey, forwardReq);

    if (body.stream) {
      if (!cloudResponse.body) {
        return c.json({ error: 'No response body from cloud' }, 502);
      }
    ...
    if (openai.hasToolCalls(responseData)) {
      ...
      log('intercept', `Cloud requested ${toolCalls.length} tool call(s): ${toolCalls.map((t) => t.function.name).join(', ')}`);
      ...
      const continuationReq: openai.OpenAIRequest = {
        ...forwardReq,
        messages: continuationMessages as openai.OpenAIRequest['messages'],
      };

      const continuationRes = await openai.forwardToCloud(config.cloud.openai_base, apiKey, continuationReq);
      const finalData = await continuationRes.json();
...
async function handleAnthropicRequest(
  ...
  if (!decision.shouldIntercept) {
    stats.passedThrough++;
    return forwardAnthropicPassthrough(config.cloud.anthropic_base, apiKey, body, c);
  }
  ...
  try {
    ...
    const cloudResponse = await anthropic.forwardToCloud(config.cloud.anthropic_base, apiKey, forwardReq);

    if (body.stream && cloudResponse.body) {
      const headers = createSSEHeaders();
      for (const [k, v] of Object.entries(headers)) {
        c.header(k, v);
      }
...
async function forwardOpenAIPassthrough(
  ...
  try {
    const res = await openai.forwardToCloud(cloudBase, apiKey, body);
...
async function forwardAnthropicPassthrough(
  ...
  try {
    const res = await anthropic.forwardToCloud(cloudBase, apiKey, body);
    if (body.stream && res.body) {
      const headers = createSSEHeaders();
      for (const [k, v] of Object.entries(headers)) {
        c.header(k, v);
